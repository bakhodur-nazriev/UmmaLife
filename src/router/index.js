...
const routes = [
	...
	{
		path: '/:lang?',
		name: 'redirect-reset-or-forgot-password',
		beforeEnter: (to, from, next) => {
			const linkParam = to.query.link1
			if (linkParam === 'reset-password') {
				next({name: 'ResetPasswordView', params: to.params})
			} else {
				next({name: 'forgot-password', params: to.params})
			}
		},
		meta: {
			title: 'Redirect to Reset or Forgot Password',
			requiresAuth: false,
			layout: 'auth'
		}
	}
